...
{	//1 tick is 0.5 uS 
 ...
 TL0 = (0x0F&us);  //TL0 = LOBYTE(us);                       //set low byte of period
 TH0 = ((0xF0&us)>>4);  //TH0 = HIBYTE(us);                       //set hight byte of period
 TR0=1; //set_TCON_TR0;                           //Start Timer0
 while (TF0 != 1);                       //Wait Timer0 Time-Out Flag
 TR0=0; //clr_TCON_TR0;                           //Stop Timer0
 TF0=0; //clr_TCON_TF0;                           //clear flag	
}
...
void cmt_spi3_sdio_out(void){P01_PushPull_Mode;} //set SDIO as output
void cmt_spi3_sdio_in(void) //set SDIO as input
{
 	
 P01_Quasi_Mode; //quasi mode with puu-up
 P01 = 1; //for reading output must be set as 1
}
